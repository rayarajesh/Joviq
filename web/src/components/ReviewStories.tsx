...
export function ReviewStories({ testimonials }: { testimonials: Story[] }) {
  ...
  const [discipline, setDiscipline] = useState("All stories");
  const featured = testimonials[featuredIndex];
  const disciplines = ["All stories", ...new Set(testimonials.map(story => story.program.split(" - ")[0]))];
  const visible = testimonials.filter(story => discipline === "All stories" || story.program.split(" - ")[0] === discipline);
