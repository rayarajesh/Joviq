...
import {
  studentPreviewDashboard,
  studentPreviewWorkspace,
} from "../data/studentPreview";
...
import {
  ...
  assetTypes,
  assetVisibilities,
} from "../features/assets/api/assetsTypes";
...
const adminNavGroups: DashboardNavGroup[] = [
  ...
  {
    label: "Learning",
    items: ["Categories", "Programs", "Curriculum", "Projects", "Certificates"],
  },
...
export function DashboardPage({
  ...
  const primaryRole: PrimaryRole =
    !isPreview && roles.includes("Admin") ? "Admin" : "Student";
  const requestedModule = searchParams.get("section");
  const normalizedRequestedModule =
    requestedModule === "Dashboard" ? "Overview" : requestedModule;
  const canOpenRequestedModule =
  ...
  return (
    <main
      className={`dashboard-shell dashboard-shell--${primaryRole.toLowerCase()}`}
    >
      <DashboardSidebar
      ...
      <nav className="sidebar-nav" aria-label="Dashboard navigation">
        {navGroups.map((group) => (
          <div
            ...
            key={group.label ?? group.items.join("-")}
          >
            {group.label ? (
...
function AdminDashboard({
  ...
  const [studentQuery, setStudentQuery] = useState<PeopleQueryState>(() => ({
    ...initialPeopleQuery,
  }));
  const [studentsPage, setStudentsPage] =
    useState<PagedResult<AdminUserResponse> | null>(null);
  const [summary, setSummary] = useState<AdminUserSummaryResponse | null>(null);
  const [lmsSummary, setLmsSummary] = useState<AdminLmsSummaryResponse | null>(
    null,
  );
  ...
  const [programCategories, setProgramCategories] = useState<
    ProgramCategoryResponse[]
  >([]);
  const [adminCurriculum, setAdminCurriculum] = useState<
    CurriculumModuleResponse[]
  >([]);
  ...
  const [creatingRole, setCreatingRole] = useState<AssignableRoleName | null>(
    null,
  );
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      if (studentsResponse.status === "fulfilled")
        setStudentsPage(studentsResponse.value.data);
      if (lmsSummaryResponse.status === "fulfilled")
        setLmsSummary(lmsSummaryResponse.value.data);
      if (lmsProgramsResponse.status === "fulfilled")
        setLmsPrograms(lmsProgramsResponse.value.data);
      if (categoriesResponse.status === "fulfilled")
        setProgramCategories(categoriesResponse.value.data);
      if (curriculumResponse.status === "fulfilled")
        setAdminCurriculum(curriculumResponse.value.data);
      if (projectsResponse.status === "fulfilled")
        setAdminProjects(projectsResponse.value.data);
      if (enrollmentsResponse.status === "fulfilled")
        setAdminEnrollments(enrollmentsResponse.value.data);
      if (paymentsResponse.status === "fulfilled")
        setAdminPayments(paymentsResponse.value.data);
      if (couponsResponse.status === "fulfilled")
        setAdminCoupons(couponsResponse.value.data);
      if (certificatesResponse.status === "fulfilled")
        setAdminCertificates(certificatesResponse.value.data);
      if (notificationsResponse.status === "fulfilled")
        setAdminNotifications(notificationsResponse.value.data);
      if (auditLogsResponse.status === "fulfilled")
        setAuditLogs(auditLogsResponse.value.data.items);
      const results = [
        { name: "Summary", result: summaryResponse },
        { name: "Students Page", result: studentsResponse },
        { name: "Lms Summary", result: lmsSummaryResponse },
        { name: "Lms Programs", result: lmsProgramsResponse },
        { name: "Program Categories", result: categoriesResponse },
        { name: "Admin Curriculum", result: curriculumResponse },
        { name: "Admin Projects", result: projectsResponse },
        { name: "Admin Enrollments", result: enrollmentsResponse },
        { name: "Admin Payments", result: paymentsResponse },
        { name: "Admin Coupons", result: couponsResponse },
        { name: "Admin Certificates", result: certificatesResponse },
        { name: "Admin Notifications", result: notificationsResponse },
      ];
      ...
      if (failed.length)
        setMessage({
          tone: "error",
          text: `Some data is unavailable: ${failed.map((item) => item.name).join(", ")}. The server could not load these records.`,
        });
    } catch (error) {
  ...
  async function runUserAction(
    ...
    try {
      ...
      setMessage({
        tone: "success",
        text: actionMessage(action, user.fullName),
      });
      await loadDashboard();
    } catch (error) {
      setMessage({ tone: "error", text: formatApiError(error) });
  ...
  async function saveUserEdit(
    ...
    const accountStatus = String(
      form.get("accountStatus") ?? user.accountStatus,
    );
    const role = String(
      form.get("role") ?? getAssignableUserRole(user),
    ) as AssignableRoleName;
...
function AdminOverview({
  ...
  const kpis = [
    ...
    },
    {
      ...
      detail: `${pendingUsers} pending email`,
      tone: "blue",
    },
    {
      icon: Tags,
    ...
    },
    {
      ...
      detail: `${activeEnrollments} active`,
      tone: "teal",
    },
    {
      icon: FolderKanban,
...
function AdminLmsPanel({
  ...
  const [programDialogMode, setProgramDialogMode] = useState<
    "create" | "edit" | null
  >(null);
  const [programEditor, setProgramEditor] =
    useState<ProgramDetailsResponse | null>(null);
  const [programEditorLoadingId, setProgramEditorLoadingId] = useState<
    string | null
  >(null);
  ...
  const [programThumbnailFile, setProgramThumbnailFile] = useState<File | null>(
    null,
  );
  ...
  const [projectDialogMode, setProjectDialogMode] = useState<
    "create" | "edit" | null
  >(null);
  const [projectEditor, setProjectEditor] = useState<ProjectResponse | null>(
    null,
  );
  const [projectReferenceFile, setProjectReferenceFile] = useState<File | null>(
    null,
  );
  const [isSavingProject, setIsSavingProject] = useState(false);
  const [projectAudience, setProjectAudience] =
    useState<ProjectResponse | null>(null);
  const [projectStudents, setProjectStudents] = useState<
    ProjectStudentResponse[]
  >([]);
  const [selectedProjectStudentIds, setSelectedProjectStudentIds] = useState<
    string[]
  >([]);
  const [projectStudentSearch, setProjectStudentSearch] = useState("");
  const [isProjectStudentsLoading, setIsProjectStudentsLoading] =
    useState(false);
  const [isPublishingProject, setIsPublishingProject] = useState(false);
  const [projectReviewProject, setProjectReviewProject] =
    useState<ProjectResponse | null>(null);
  const [projectReviews, setProjectReviews] = useState<
    ProjectSubmissionReviewResponse[]
  >([]);
  const [isProjectReviewsLoading, setIsProjectReviewsLoading] = useState(false);
  const [projectReviewDrafts, setProjectReviewDrafts] = useState<
    Record<
      string,
      { status: "NeedsRevision" | "Approved"; score: string; feedback: string }
    >
  >({});
  const [isReviewingProject, setIsReviewingProject] = useState<string | null>(
    null,
  );
  ...
  const [enrollmentCategoryFilter, setEnrollmentCategoryFilter] =
    useState("All");
  const [enrollmentProgramFilter, setEnrollmentProgramFilter] = useState("All");
  const [enrollmentStatusFilter, setEnrollmentStatusFilter] = useState("All");
  const [paymentSearch, setPaymentSearch] = useState("");
  const [paymentStatusFilter, setPaymentStatusFilter] = useState("All");
  const hasOpenAdminDialog = Boolean(
    categoryDialogMode ||
    programDialogMode ||
    planEditorProgram ||
    projectDialogMode ||
    projectAudience ||
    projectReviewProject,
  );
    ...
    return programs
      .filter(
        (program) =>
          ...
          program.categoryId === enrollmentCategoryFilter,
      )
  ...
  const enrollmentStats = useMemo(() => {
    ...
    return {
      ...
      active: adminEnrollments.filter(
        (enrollment) => enrollment.status === "Active",
      ).length,
      reserved: adminEnrollments.filter(
        (enrollment) => enrollment.status === "Reserved",
      ).length,
      completed: adminEnrollments.filter(
        (enrollment) => enrollment.status === "Completed",
      ).length,
      expiringSoon,
      collected: adminEnrollments.reduce(
        (total, enrollment) => total + enrollment.paidAmount,
        0,
      ),
    };
  }, [adminEnrollments]);
  ...
  const visibleEnrollments = useMemo(() => {
    ...
    return adminEnrollments.filter((enrollment) => {
      const matchesCategory =
        enrollmentCategoryFilter === "All" ||
        enrollmentProgramCategoryById.get(enrollment.programId) ===
          enrollmentCategoryFilter;
      const matchesProgram =
        enrollmentProgramFilter === "All" ||
        enrollment.programId === enrollmentProgramFilter;
      const matchesStatus =
  ...
  async function createProgram(event: FormEvent<HTMLFormElement>) {
    ...
    if (!categoryId || !title) {
      onMessage({
        tone: "error",
        text: "Choose a category and enter a program title.",
      });
      return;
    }
    ...
    try {
      await adminLmsApi.createProgram({
        ...
        learningMode: String(
          form.get("learningMode") ?? "Live + recorded + project practice",
        ),
        ...
        faqs: [
          {
            ...
            answer:
              "Yes. The program starts from foundations and moves into projects.",
          },
          {
            question: "Will I get certification?",
            answer: "Yes. Certification is linked to completed project work.",
          },
        ],
        status: 2,
      });
  ...
  async function openProgramDialog(program?: ProgramSummaryResponse) {
    ...
    if (!program) {
      ...
      setProgramThumbnailUrl(
        selectedProgramCategory
          ? getProgramImage("", selectedProgramCategory.name)
          : "",
      );
    ...
    try {
      ...
      setProgramThumbnailUrl(
        toApiAcceptableThumbnailUrl(response.data.thumbnailUrl) ||
          getProgramImage(response.data.slug, response.data.categoryName),
      );
  ...
  function chooseProgramThumbnailFile(file: File | null) {
    ...
    if (file.size > 10 * 1024 * 1024) {
      onMessage({
        tone: "error",
        text: "Thumbnail image must be 10 MB or smaller.",
      });
      return;
    }
  ...
  async function uploadProgramThumbnail(programId: string, file: File) {
    ...
    if (uploadResponse.data.deliveryUrl || uploadResponse.data.publicUrl) {
      return (
        toApiAcceptableThumbnailUrl(
          uploadResponse.data.deliveryUrl ??
  ...
  async function saveProgram(event: FormEvent<HTMLFormElement>) {
    ...
    const selectedThumbnailUrl =
      toApiAcceptableThumbnailUrl(programThumbnailUrl);
    const payload = {
      categoryId,
      title,
      ...
      overview:
        ...
        programEditor?.overview ||
        fallbackOverview,
      level:
        String(form.get("level") ?? "").trim() ||
        programEditor?.level ||
        "Beginner to job-ready",
      duration:
        String(form.get("duration") ?? "").trim() ||
        programEditor?.duration ||
        "8 to 16 weeks",
      learningMode:
    ...
    if (
      projectPayloads.some((project) => !project.title || !project.description)
    ) {
      onMessage({
        tone: "error",
        text: "Complete or remove every project before saving the program.",
      });
    ...
    try {
      ...
      if (programEditor) {
        ...
        if (programThumbnailFile) {
          const uploadedThumbnailUrl = await uploadProgramThumbnail(
            response.data.id,
            programThumbnailFile,
          );
      ...
      if (savedProgramId) {
        ...
        const retainedProjectIds = new Set(
          projectPayloads.flatMap((project) =>
          ...
          existingProjects
            .filter(
              (project) =>
                ...
                !projectPayloads.some((item) => item.id === project.id),
            )
      ...
      onMessage({
        tone: "success",
        text: programEditor ? "Program updated." : `${title} program created.`,
      });
      await onRefresh();
    } catch (error) {
      onMessage({ tone: "error", text: formatApiError(error) });
  ...
  async function updateProgramPlan(
    event: FormEvent<HTMLFormElement>,
    plan: ProgramPlanResponse,
  ) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
  ...
  async function restoreMissingPlans() {
    ...
    const existingCodes = new Set(
      planEditorProgram.plans.map((plan) => plan.code),
    );
    const missingPlans = defaultProgramPlans.filter(
      (plan) => !existingCodes.has(plan.code),
    );

    if (missingPlans.length === 0) {
      onMessage({
        tone: "success",
        text: "All three plans are already configured.",
      });
      return;
    }
    ...
    try {
      await Promise.all(
        missingPlans.map((plan) =>
          adminLmsApi.createPlan(planEditorProgram.id, {
            name: plan.name,
            code: plan.code,
  ...
  async function saveCategory(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      onMessage({
        tone: "success",
        text: categoryEditor ? "Category updated." : "Category created.",
      });
      await onRefresh();
    } catch (error) {
      onMessage({ tone: "error", text: formatApiError(error) });
  ...
  async function saveProject(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      title,
      description,
      requiredArtifacts: parseCommaList(
        String(form.get("requiredArtifacts") ?? ""),
      ),
      usefulLinks,
    ...
    if (!programId || !title || !description) {
      onMessage({
        tone: "error",
        text: "Choose a program and complete the project title and description.",
      });
      return;
    }
    ...
    try {
      ...
      if (projectReferenceFile) {
        const referenceMediaUrl = await uploadProjectReference(
          projectReferenceFile,
          programId,
        );
        savedProject = (
          await adminLmsApi.updateProject(savedProject.id, {
            ...payload,
            referenceMediaUrl,
            isPublished: false,
          })
      ...
      if (wantsPublish) {
        onMessage({
          tone: "success",
          text: "Project saved. Choose the active students who should receive it.",
        });
      ...
      setProjectReviewDrafts(
        Object.fromEntries(
          response.data.map((review) => [
            ...
            {
              status:
                review.submission.status === "NeedsRevision"
                  ? "NeedsRevision"
                  : "Approved",
              score:
                review.submission.score === undefined ||
                review.submission.score === null
                  ? ""
                  : String(review.submission.score),
  ...
  async function reviewProjectSubmission(
    review: ProjectSubmissionReviewResponse,
  ) {
    ...
    if (draft.status === "Approved" && !draft.score.trim()) {
      onMessage({
        tone: "error",
        text: "Add the awarded score before approving the submission.",
      });
      return;
    }
    ...
    if (
      score !== undefined &&
      (!Number.isFinite(score) || score < 0 || score > review.maxScore)
    ) {
      onMessage({
        tone: "error",
        text: `Score must be between 0 and ${review.maxScore}.`,
      });
      return;
    }
    ...
    try {
      ...
      onMessage({
        tone: "success",
        text: `${review.studentName}'s submission was reviewed.`,
      });
      const refreshed = await adminLmsApi.getProjectSubmissions(
        review.projectId,
      );
      ...
      setProjectReviewDrafts(
        Object.fromEntries(
          refreshed.data.map((item) => [
            ...
            {
              status:
                item.submission.status === "NeedsRevision"
                  ? "NeedsRevision"
                  : "Approved",
              score:
  ...
  function toggleProjectStudent(studentId: string) {
    setSelectedProjectStudentIds((selected) =>
  ...
  function toggleAllProjectStudents() {
    const visibleIds = filteredProjectStudents.map(
      (student) => student.studentId,
    );
    const allVisibleSelected =
      visibleIds.length > 0 &&
      visibleIds.every((id) => selectedProjectStudentIds.includes(id));
    setSelectedProjectStudentIds((selected) =>
      allVisibleSelected
  ...
  async function publishProject() {
    if (!projectAudience || selectedProjectStudentIds.length === 0) {
      onMessage({
        tone: "error",
        text: "Select at least one active student before publishing.",
      });
      return;
    }
    ...
    try {
      await adminLmsApi.publishProject(projectAudience.id, {
        studentIds: selectedProjectStudentIds,
      });
      closeProjectAudience();
      onMessage({
        tone: "success",
        text: `Project published to ${selectedProjectStudentIds.length} student${selectedProjectStudentIds.length === 1 ? "" : "s"}.`,
      });
      await onRefresh();
    } catch (error) {
      onMessage({ tone: "error", text: formatApiError(error) });
  ...
  async function deleteProject(project: ProjectResponse) {
    if (
      !window.confirm(
        `Delete “${project.title}”? This will also remove its student submissions.`,
      )
    ) {
      return;
    }
  ...
  async function failPayment(payment: PaymentTransactionResponse) {
    const reason = window.prompt(
      "Why is this payment being marked failed?",
      payment.failureReason ?? "Payment could not be confirmed.",
    );
    if (reason === null) {
      return;
    }
  ...
  function selectCertificateEnrollment(enrollmentId: string) {
    const enrollment = adminEnrollments.find(
  ...
  async function issueCertificate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await adminLmsApi.issueCertificate({
        ...
        authorizedSignatory:
          certificateDraft.authorizedSignatory?.trim() || undefined,
        signatureText: certificateDraft.signatureText?.trim() || undefined,
      });
      onMessage({
        tone: "success",
        text: "Certificate issued and added to the certificate register.",
      });
      await onRefresh();
    } catch (error) {
      onMessage({ tone: "error", text: formatApiError(error) });
  ...
  const selectedProgramCategory = selectedCategorySlug
    ? categories.find(
        (category) =>
          category.slug.toLowerCase() === selectedCategorySlug.toLowerCase(),
      )
    : null;
  const selectedProgramId = searchParams.get("programId");
  const visiblePrograms = selectedProgramCategory
    ? programs.filter(
        (program) => program.categoryId === selectedProgramCategory.id,
      )
    : programs;
  const categoryProgramCount = categories.reduce(
    (total, category) => total + category.programs.length,
    0,
  );
  const visibleProgramCounts = {
    total: visiblePrograms.length,
    published: visiblePrograms.filter(
      (program) => program.status === "Published",
    ).length,
    draft: visiblePrograms.filter((program) => program.status === "Draft")
      .length,
    archived: visiblePrograms.filter((program) => program.status === "Archived")
      .length,
  };
  ...
  const allVisibleProjectStudentsSelected =
    filteredProjectStudents.length > 0 &&
  ...
  return (
    <section
      className={`dashboard-card lms-admin-panel${usesCatalogModuleHero ? " lms-admin-panel--categories" : ""}${isCurriculumModule ? " lms-admin-panel--curriculum" : ""}`}
    >
      {usesCatalogModuleHero ? (
        <div className="category-page-header">
          <div>
            <span className="eyebrow">
              ...
              {isProgramsModule ? "Programs" : "Categories"}
            </span>
          ...
          <button
            ...
            type="button"
            onClick={() =>
              isProgramsModule ? void openProgramDialog() : openCategoryDialog()
            }
          >
            <Plus size={18} />
        ...
        <div className="card-title-row">
          <div>
            ...
            <h2>{activeModule}</h2>
            <p>
              {summary?.publishedPrograms ?? 0} published programs,{" "}
              {summary?.activeEnrollments ?? 0} active enrollments.
            </p>
          </div>
          <Layers3 size={23} />
        </div>
      ...
      {workflowInfo && (
        <div className="admin-workflow-backdrop">
          <dialog
            className="admin-workflow-dialog"
            ref={(node) => {
              if (node && !node.open) node.showModal();
            }}
            onCancel={() => setWorkflowInfo(null)}
            aria-labelledby="workflow-title"
          >
            <button
              className="admin-dialog-close"
              aria-label="Close workflow"
              onClick={() => setWorkflowInfo(null)}
            >
              <X size={20} />
            </button>
      ...
      >
        {showModule("Categories") ? (
          <section className="category-admin-panel">
            <div className="category-admin-grid">
              {categories.length === 0 ? (
                <div className="table-state">No categories yet.</div>
              ) : null}
                ...
                >
                    ...
                    <div className="category-admin-card__title-row">
                      ...
                      <div>
                        ...
                        <small>
                          ...
                          {category.isPublished === false ? " � Inactive" : ""}
                        </small>
                      </div>
                    </div>
                  ...
                  <div className="category-admin-card__footer">
                    ...
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/dashboard?section=Programs&category=${encodeURIComponent(category.slug)}`,
                        )
                      }
                    >
        ...
        {showModule("Categories") && categoryDialogMode
          ? createPortal(
              <div
                ...
                >
                  <div className="category-dialog__header">
                    ...
                    <div>
                      ...
                      <p>
                        {categoryDialogMode === "edit"
                          ? "Update how this category appears across admin and website pages."
                          : "Create a clean learning domain for programs on the website."}
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  >
                    <label>
                      ...
                      <input
                        name="name"
                        defaultValue={categoryEditor?.name ?? ""}
                        minLength={2}
                    ...
                    <label>
                      ...
                      <select
                        name="isPublished"
                        defaultValue={
                          categoryEditor?.isPublished === false
                            ? "false"
                            : "true"
                        }
                      >
        ...
        {isProgramsModule ? (
          <section className="program-admin-panel">
            <div
              className="program-admin-summary"
              aria-label="Program status summary"
            >
              <span>
            ...
            {selectedProgramCategory ? (
              <div className="program-filter-banner">
                ...
                <button
                  type="button"
                  onClick={() => navigate("/dashboard?section=Programs")}
                >
                  Show all programs
                </button>
              </div>
            ...
            <div className="program-admin-grid">
              {visiblePrograms.length === 0 ? (
                <div className="table-state">No programs found.</div>
              ) : null}
                ...
                >
                  <div className="program-admin-card__media">
                    <img
                      ...
                      onError={(event) => {
                        ...
                        event.currentTarget.src = getProgramImage(
                          program.slug,
                          program.categoryName,
                        );
                      }}
                      src={
                        toApiAcceptableThumbnailUrl(program.thumbnailUrl) ||
                        getProgramImage(program.slug, program.categoryName)
                      }
                    />
                    <div className="program-admin-card__media-actions">
                      <span className={statusClassName(program.status)}>
                  ...
                  <div className="program-admin-card__footer">
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/dashboard?section=Curriculum&programId=${program.id}`,
                        )
                      }
                    >
                      <ListChecks size={16} />
        ...
        {isProgramsModule && programDialogMode
          ? createPortal(
              <div
                className="category-dialog-backdrop"
                role="presentation"
              ...
              >
                <section
                  className="category-dialog program-dialog"
                  role="dialog"
                  aria-modal="true"
                ...
                >
                  <div className="category-dialog__header">
                    ...
                    <div>
                      ...
                      <p>
                        {programDialogMode === "edit"
                          ? "Update the public program card, status, and learning details."
                          : "Create a program and keep it ready for curriculum and pricing setup."}
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  <form
                    ...
                    key={
                      programEditor?.id ??
                      selectedProgramCategory?.id ??
                      "create-program"
                    }
                    onSubmit={saveProgram}
                  >
                    <div
                      className="program-dialog__steps"
                      aria-label="Program setup steps"
                    >
                      <span
                        className={programDialogStep === 1 ? "is-active" : ""}
                      >
                        1. Program details
                      </span>
                      <span
                        className={programDialogStep === 2 ? "is-active" : ""}
                      >
                        2. Projects
                      </span>
                    </div>
                    <div
                      className={`program-dialog__step-one${programDialogStep === 1 ? "" : " is-hidden"}`}
                    >
                      <div className="program-thumbnail-field">
                        <div className="program-thumbnail-field__preview">
                          <img
                            ...
                            onError={(event) => {
                              ...
                              event.currentTarget.src = getProgramImage(
                                programEditor?.slug ?? "",
                                programEditor?.categoryName ??
                                  selectedProgramCategory?.name ??
                                  "",
                              );
                            }}
                            src={
                              programThumbnailPreviewUrl ||
                              programThumbnailUrl ||
                              getProgramImage(
                                programEditor?.slug ?? "",
                                programEditor?.categoryName ??
                        ...
                        <div className="program-thumbnail-field__controls">
                          <div>
                            <span>Thumbnail</span>
                            <strong>
                              Choose from media or upload an image
                            </strong>
                          </div>
                          <div className="program-thumbnail-media">
                            {programThumbnailOptions.map((option) => (
                              <button
                                key={option.url}
                                className={
                                  programThumbnailUrl === option.url
                                    ? "is-selected"
                                    : undefined
                                }
                                ...
                                type="button"
                                onClick={() =>
                                  chooseProgramThumbnailUrl(option.url)
                                }
                              >
                          ...
                          {programThumbnailFile ? (
                            <small>{programThumbnailFile.name} selected</small>
                          ) : null}
                        </div>
                      </div>
                      <label>
                      ...
                      <label>
                        ...
                        <select
                          name="status"
                          defaultValue={programStatusValue(
                            programEditor?.status ?? "Published",
                          )}
                        >
                      ...
                      <label>
                        ...
                        <input
                          name="title"
                          defaultValue={programEditor?.title ?? ""}
                      ...
                      <label>
                        ...
                        <input
                          name="duration"
                          defaultValue={
                            programEditor?.duration ?? "8 to 16 weeks"
                          }
                        />
                      </label>
                      <label>
                        ...
                        <input
                          name="level"
                          defaultValue={
                            programEditor?.level ?? "Beginner to job-ready"
                          }
                        />
                      </label>
                      <label>
                      ...
                      <label>
                        ...
                        <input
                          name="skills"
                          defaultValue={programEditor?.skills.join(", ") ?? ""}
                          placeholder="Python, SQL, Projects"
                        />
                      </label>
                      <label>
                        ...
                        <input
                          ...
                          defaultValue={programEditor?.certificationName ?? ""}
                          placeholder="Joviq program certification"
                        />
                      </label>
                      <label className="program-dialog__wide-field">
                        <span>Outcomes</span>
                    ...
                    <div
                      className={`program-dialog__step-two${programDialogStep === 2 ? "" : " is-hidden"}`}
                    >
                      <div className="program-project-step">
                        <div className="program-project-step__heading">
                          <div>
                            ...
                            <p>
                              Add the project cards students should see for this
                              program.
                            </p>
                          </div>
                          <button
                        ...
                        <div className="program-project-list">
                          {programProjects.map((project, index) => (
                            <div
                              className="program-project-row"
                              key={project.id ?? `new-${index}`}
                            >
                              <span className="program-project-row__number">
                                {index + 1}
                              </span>
                              <label>
                          ...
                          {programProjects.length === 0 ? (
                            <p className="table-state">
                              No projects added yet. Use Add project to create
                              the first card.
                            </p>
                          ) : null}
                        </div>
                      ...
                      {programDialogStep === 1 ? (
                        ...
                        <>
                          <button
                            className="secondary-action"
                            type="button"
                            onClick={() => setProgramDialogStep(1)}
                          >
                            Back
                          </button>
                          <button
                            ...
                            type="submit"
                            disabled={
                              isSavingProgram || categories.length === 0
                            }
                          >
                            <Save size={18} />
        ...
        {showPrograms && !isProgramsModule ? (
          <form className="lms-mini-form" onSubmit={createProgram}>
            ...
            <label>
              ...
              <input
                name="learningMode"
                defaultValue="Live + recorded + project practice"
              />
            </label>
            <label>
              Skills
              <input
                name="skills"
                placeholder="Python, SQL, Projects, Interview prep"
              />
            </label>
            <button
              className="primary-action"
              type="submit"
              disabled={isCreatingProgram || categories.length === 0}
            >
              <UserPlus size={18} />
              ...
              {programs.map((program) => (
                <article key={program.id} className="lms-list-item">
                  <div>
                    <strong>{program.title}</strong>
                    <span>
                      {program.categoryName} - {program.duration}
                    </span>
                  </div>
        ...
        {activeModule === "Programs" && planEditorProgram
          ? createPortal(
              ...
              >
                <section
                  className="category-dialog admin-plan-dialog"
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="admin-plan-dialog-title"
                >
                  <div className="admin-plan-editor" id="admin-plan-editor">
                    <div className="admin-plan-editor__header">
                      ...
                      <div>
                        ...
                        <h3 id="admin-plan-dialog-title">
                          {planEditorProgram.title}
                        </h3>
                        <p>
                          Update plan prices, access amount, features, and
                          website visibility.
                        </p>
                      </div>
                      <div className="admin-plan-editor__actions">
                        <button
                          type="button"
                          onClick={() => void restoreMissingPlans()}
                        >
                          <RotateCcw size={16} />
                          Restore plans
                        </button>
                    ...
                    <div className="admin-plan-editor__grid">
                      {planEditorProgram.plans.map((plan) => (
                        <form
                          ...
                          onSubmit={(event) =>
                            void updateProgramPlan(event, plan)
                          }
                        >
                          ...
                          <label>
                            ...
                            <input
                              ...
                              type="number"
                              min="0"
                              defaultValue={plan.reserveAmount}
                              required
                            />
                          </label>
                          <label>
                      ...
                      {planEditorProgram.plans.length === 0 ? (
                        <div className="table-state">
                          No plans configured. Use Restore plans to create all
                          three.
                        </div>
                      ) : null}
                    </div>
          ...
          <section className="project-admin-workspace">
            <div className="project-admin-hero">
              <div>
                ...
                <p>
                  Create a brief, attach helpful references, set the rubric,
                  then publish it only to active students in the program.
                </p>
              </div>
              <button
                className="primary-action project-create-button"
                type="button"
            ...
            <div className="project-admin-stats" aria-label="Project summary">
              ...
              <span>
                <strong>
                  {
                    adminProjects.filter((project) => project.isPublished)
                      .length
                  }
                </strong>
              ...
              <span>
                <strong>
                  {adminProjects.reduce(
                    (total, project) =>
                      total + (project.assignedStudentCount ?? 0),
                    0,
                  )}
                </strong>
                <small>Student assignments</small>
              </span>
            </div>
            ...
            <div className="project-admin-grid">
              {adminProjects.length === 0 ? (
                <div className="project-empty-state">
                  <span className="project-empty-state__icon">
                    <FolderKanban size={24} />
                  </span>
                  <strong>Your project board is ready.</strong>
                  <p>
                    Start with a clear brief and give students the references
              ...
              {adminProjects.map((project) => {
                const programTitle =
                  programs.find((program) => program.id === project.programId)
                    ?.title ?? "Program not found";
                const usefulLinks = project.usefulLinks ?? [];
                return (
                  <article
                    key={project.id}
                    className={`project-admin-card${project.isPublished ? " is-published" : " is-draft"}`}
                  >
                    <div className="project-admin-card__topline">
                      <span className="project-program-label">
                        <FolderKanban size={15} /> {programTitle}
                      </span>
                      <span
                        className={`project-status-badge${project.isPublished ? " is-published" : " is-draft"}`}
                      >
                    ...
                    <div className="project-admin-card__meta">
                      ...
                      <span>
                        <Trophy size={15} /> {project.maxScore} points
                      </span>
                      <span>
                        <UsersRound size={15} />{" "}
                        {project.assignedStudentCount ?? 0} assigned
                      </span>
                    </div>
                      ...
                      <div className="project-admin-card__links">
                        {usefulLinks.map((link) => (
                          <a
                            ...
                            href={toApiAcceptableThumbnailUrl(link.url)}
                            target="_blank"
                            rel="noreferrer"
                          >
                    ...
                    {project.referenceMediaUrl ? (
                      <a
                        className="project-reference-chip"
                        href={toApiAcceptableThumbnailUrl(
                          project.referenceMediaUrl,
                        )}
                        target="_blank"
                        rel="noreferrer"
                      >
                        <FileText size={15} /> View reference media{" "}
                        <ExternalLink size={13} />
                      </a>
                    ) : null}
                    <div className="project-admin-card__actions">
                      ...
                      <button
                        type="button"
                        onClick={() => void openProjectReviews(project)}
                      >
                        <ClipboardList size={16} /> Review submissions
                      </button>
                      <button
                        type="button"
                        onClick={() => void openProjectAudience(project)}
                      >
                        <UsersRound size={16} />{" "}
                        {project.isPublished
                          ? "Manage students"
        ...
        {showModule("Projects") && projectDialogMode
          ? createPortal(
              <div
                className="category-dialog-backdrop"
                role="presentation"
              ...
              >
                ...
                >
                  <div className="project-editor-dialog__header">
                    <span className="project-editor-dialog__icon">
                      <FolderKanban size={22} />
                    </span>
                    <div>
                      <span className="eyebrow">Project brief</span>
                      <h3 id="project-editor-dialog-title">
                        {projectDialogMode === "edit"
                          ? "Edit project"
                          : "Create project"}
                      </h3>
                      <p>
                        Give students a focused outcome, practical references,
                  ...
                  <form
                    className="project-editor-dialog__form"
                    key={projectEditor?.id ?? "create-project"}
                    onSubmit={saveProject}
                  >
                    <div className="project-form-section">
                      <div className="project-form-section__heading">
                        <span>01</span>
                        <div>
                          <strong>Project basics</strong>
                          <small>What should students build?</small>
                        </div>
                      </div>
                      <label>
                        <span>Program</span>
                        <select
                          name="programId"
                          defaultValue={projectEditor?.programId ?? ""}
                          required
                        >
                      ...
                      <label>
                        ...
                        <input
                          name="title"
                          defaultValue={projectEditor?.title ?? ""}
                          minLength={2}
                          placeholder="Example: Build a finance KPI dashboard"
                          required
                        />
                      </label>
                      <label>
                        ...
                        <textarea
                          name="description"
                          defaultValue={projectEditor?.description ?? ""}
                          minLength={10}
                          placeholder="Explain the outcome, context, and what a good submission should demonstrate."
                          required
                        />
                      </label>
                    </div>
                    ...
                    <div className="project-form-section">
                      <div className="project-form-section__heading">
                        <span>02</span>
                        <div>
                          <strong>Helpful references</strong>
                          <small>Links and media students can use</small>
                        </div>
                      </div>
                        ...
                        {[0, 1, 2].map((index) => (
                          <div className="project-link-row" key={index}>
                            <input
                              name="linkLabel"
                              defaultValue={
                                projectEditor?.usefulLinks?.[index]?.label ?? ""
                              }
                              placeholder={
                                index === 0
                                  ? "GitHub"
                              ...
                              aria-label={`Reference link ${index + 1} label`}
                            />
                            <input
                              name="linkUrl"
                              type="url"
                              defaultValue={
                                projectEditor?.usefulLinks?.[index]?.url ?? ""
                              }
                              placeholder="https://..."
                      ...
                      <label>
                        ...
                        <input
                          ...
                          defaultValue={
                            projectEditor?.requiredArtifacts?.join(", ") ??
                            "GitHub link, Demo, Report"
                          }
                          placeholder="GitHub link, Demo, Report"
                        />
                      </label>
                      <label className="project-upload-field">
                        <span>
                          Reference media{" "}
                          <small>(image, video, PDF, or document)</small>
                        </span>
                        <span className="project-upload-control">
                          <UploadCloud size={18} />
                          <span>
                            {projectReferenceFile?.name ??
                              (projectEditor?.referenceMediaUrl
                                ? "Replace attached reference media"
                          ...
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              if (file && file.size > 25 * 1024 * 1024) {
                                onMessage({
                                  tone: "error",
                                  text: "Reference media must be 25 MB or smaller.",
                                });
                                event.currentTarget.value = "";
                                return;
                              }
                    ...
                    <div className="project-form-section project-form-section--compact">
                      <div className="project-form-section__heading">
                        <span>03</span>
                        <div>
                          <strong>Scoring and access</strong>
                          <small>Set expectations before you publish</small>
                        </div>
                      </div>
                      <div className="project-form-grid">
                        ...
                        <label>
                          ...
                          <input
                            ...
                            min="1"
                            max="100"
                            defaultValue={projectEditor?.maxScore ?? 100}
                            required
                          />
                        </label>
                      </div>
                      <label>
                        ...
                        <select
                          name="status"
                          defaultValue={
                            projectEditor?.isPublished ? "publish" : "draft"
                          }
                        >
        ...
        {showModule("Projects") && projectAudience
          ? createPortal(
              <div
                className="category-dialog-backdrop"
                role="presentation"
              ...
              >
                <section
                  className="project-audience-dialog"
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="project-audience-dialog-title"
                >
                  <div className="project-audience-dialog__header">
                    ...
                    <div>
                      ...
                      <h3 id="project-audience-dialog-title">
                        Choose active students
                      </h3>
                      <p>
                        {projectAudience.title} will appear only for the
                        students you select.
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={closeProjectAudience}
                      title="Close dialog"
                    >
                      <X size={19} />
                    </button>
                  ...
                  <div className="project-audience-dialog__content">
                    <div className="project-audience-toolbar">
                      <label className="project-student-search">
                        <Search size={17} />
                        <input
                          value={projectStudentSearch}
                          onChange={(event) =>
                            setProjectStudentSearch(event.target.value)
                          }
                      ...
                      <label className="project-select-all">
                        <input
                          type="checkbox"
                          checked={allVisibleProjectStudentsSelected}
                          onChange={toggleAllProjectStudents}
                          disabled={filteredProjectStudents.length === 0}
                        />
                    ...
                    <div className="project-audience-summary">
                      <strong>
                        {selectedProjectStudentIds.length} selected
                      </strong>
                      <span>
                        {isProjectStudentsLoading
                          ? "Loading active enrollments..."
                          : `${projectStudents.length} active student${projectStudents.length === 1 ? "" : "s"} in this program`}
                      </span>
                    </div>
                    <div className="project-student-list">
                      {isProjectStudentsLoading ? (
                        <div className="project-student-empty">
                      ...
                      filteredProjectStudents.length === 0 ? (
                        <div className="project-student-empty">
                          No active students found for this program.
                        </div>
                      ) : null}
                      {!isProjectStudentsLoading
                        ? filteredProjectStudents.map((student) => (
                            <label
                              className={`project-student-option${selectedProjectStudentIds.includes(student.studentId) ? " is-selected" : ""}`}
                              key={student.studentId}
                            >
                              <input
                                type="checkbox"
                                checked={selectedProjectStudentIds.includes(
                                  student.studentId,
                                )}
                                onChange={() =>
                                  toggleProjectStudent(student.studentId)
                                }
                              />
                              <span className="project-student-avatar">
                                {student.fullName
                                  .trim()
                                  .slice(0, 1)
                                  .toUpperCase() || "S"}
                              </span>
                              <span className="project-student-option__details">
                                <strong>{student.fullName}</strong>
                                <small>{student.email}</small>
                              </span>
                              {selectedProjectStudentIds.includes(
                                student.studentId,
                              ) ? (
                  ...
                  <div className="project-audience-dialog__actions">
                    ...
                    <button
                      ...
                      disabled={
                        isPublishingProject ||
                        isProjectStudentsLoading ||
                        selectedProjectStudentIds.length === 0
                      }
                    >
                      <Send size={17} />{" "}
                      {isPublishingProject
                        ? "Publishing..."
                        : `Publish to ${selectedProjectStudentIds.length || "selected"}`}
                    </button>
                  </div>
                </section>
              </div>,
        ...
        {showModule("Projects") && projectReviewProject
          ? createPortal(
              <div
                className="category-dialog-backdrop"
                role="presentation"
              ...
              >
                <section
                  ...
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="project-review-dialog-title"
                >
                  <div className="project-audience-dialog__header">
                    <span className="project-editor-dialog__icon">
                    ...
                    <div>
                      ...
                      <h3 id="project-review-dialog-title">
                        Review submissions
                      </h3>
                      <p>
                        {projectReviewProject.title} · award points and send
                        feedback to each student.
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={closeProjectReviews}
                      title="Close dialog"
                    >
                      <X size={19} />
                    </button>
                            ...
                            >
                              <div className="project-review-card__student">
                                ...
                                <div>
                                  ...
                                  <small>{review.studentEmail}</small>
                                </div>
                                <span
                                  className={`project-review-status is-${review.submission.status.toLowerCase()}`}
                                >
                              ...
                              <div className="project-review-card__artifacts">
                                {review.submission.fileAssetId ? (
                                  <button
                                    type="button"
                                    onClick={() =>
                                      void openProjectSubmissionFile(
                                        review.submission.fileAssetId!,
                                      )
                                    }
                                  >
                                ...
                                {review.submission.demoUrl ? (
                                  <a
                                    href={review.submission.demoUrl}
                                    target="_blank"
                                    rel="noreferrer"
                                  >
                                ...
                                <small>
                                  Submitted{" "}
                                  {formatDateTime(review.submission.createdAt)}
                                </small>
                              </div>
                              <div className="project-review-card__form">
                                ...
                                <label className="project-review-card__feedback">
                                  Message / feedback
                                  <textarea
                                    value={draft.feedback}
                                    maxLength={2500}
                                    placeholder="Share clear next steps or encouragement..."
                                    onChange={(event) =>
                                      setProjectReviewDrafts((items) => ({
                                        ...items,
                                        [review.id]: {
                              ...
                              <button
                                className="primary-action project-review-card__save"
                                type="button"
                                onClick={() =>
                                  void reviewProjectSubmission(review)
                                }
                                disabled={isReviewingProject === review.id}
                              >
                                <Save size={16} />{" "}
                                {isReviewingProject === review.id
                                  ? "Saving review..."
                                  : "Save review"}
                              </button>
                            </article>
          ...
          <section className="enrollment-admin-page">
            <section className="enrollment-admin-hero">
              <div className="enrollment-admin-hero__copy">
                <span className="enrollment-admin-eyebrow">
                  <Link to="/dashboard">Dashboard</Link> / Enrollments
                </span>
                <h2>Enrollments</h2>
                <p>
                  Track learner enrollments across all programs and keep
                  learning journeys on track.
                </p>
              </div>
              <button
                className="primary-action"
            ...
            <section
              className="enrollment-admin-kpis"
              aria-label="Enrollment summary"
            >
              <article className="enrollment-admin-kpi enrollment-admin-kpi--purple">
                <span>
                  <UsersRound size={18} /> Total enrollments
                </span>
                <strong>{enrollmentStats.total}</strong>
                <small>All learner records</small>
              </article>
              <article className="enrollment-admin-kpi enrollment-admin-kpi--green">
                <span>
                  <CheckCircle2 size={18} /> Active access
                </span>
                <strong>{enrollmentStats.active}</strong>
                <small>Access is available</small>
              </article>
              <article className="enrollment-admin-kpi enrollment-admin-kpi--gold">
                <span>
                  <WalletCards size={18} /> Pending Enrollments
                </span>
                <strong>
                  {
                    adminEnrollments.filter(
              ...
              <article className="enrollment-admin-kpi enrollment-admin-kpi--rose">
                <span>
                  <CheckCircle2 size={18} /> Completed Enrollments
                </span>
                <strong>
                  {
                    adminEnrollments.filter(
                ...
                <small>Learning completed</small>
              </article>
            </section>
            ...
            <section className="enrollment-admin-workspace">
              <div className="enrollment-admin-toolbar">
                <div>
                  ...
                  <p>
                    {visibleEnrollments.length} of {adminEnrollments.length}{" "}
                    enrollment{adminEnrollments.length === 1 ? "" : "s"} shown
                  </p>
                </div>
                <div className="enrollment-admin-toolbar__controls">
                  <button
                    className="secondary-action admin-enrollment-reset"
                    onClick={() => {
                      setEnrollmentSearch("");
                      setEnrollmentCategoryFilter("All");
                      setEnrollmentProgramFilter("All");
                  ...
                  <label className="enrollment-admin-search">
                    <Search size={17} />
                    <span className="sr-only">Search enrollments</span>
                    <input
                      value={enrollmentSearch}
                      onChange={(event) =>
                        setEnrollmentSearch(event.target.value)
                      }
                      placeholder="Search student or program"
                    />
                  </label>
                  <label className="enrollment-admin-filter">
                    <Layers3 size={16} />
                    <span className="sr-only">Filter by category</span>
                    <select
                      value={enrollmentCategoryFilter}
                      onChange={(event) => {
                        setEnrollmentCategoryFilter(event.target.value);
                        setEnrollmentProgramFilter("All");
                      }}
                    >
                  ...
                  <label className="enrollment-admin-filter">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setEnrollmentProgramFilter(event.target.value)
                      }
                    >
                      <option value="All">All programs</option>
                      {enrollmentProgramOptions.map((program) => (
                        <option key={program.id} value={program.id}>
                          {program.title}
                        </option>
                      ))}
              ...
              <div className="enrollment-admin-list">
                {visibleEnrollments.length === 0 ? (
                  <div className="enrollment-admin-empty admin-reference-empty">
                    <span className="admin-empty-art">
                    ...
                    <p>
                      Enroll students into programs to track their learning
                      journey and progress here.
                    </p>
                    <button
                      className="primary-action"
                      onClick={() => setWorkflowInfo("enrollment")}
                ...
                {visibleEnrollments.map((enrollment) => {
                  const relatedPayments = adminPayments.filter(
                  ...
                  const studentName =
                    enrollment.studentName ||
                    `Student ${enrollment.studentId.slice(0, 8)}`;
                  ...
                  return (
                    <article
                      key={enrollment.id}
                      className="enrollment-admin-card"
                    >
                      <div className="enrollment-admin-card__header">
                        <div className="enrollment-admin-card__student">
                          <span className="enrollment-admin-avatar">
                            {getInitials(studentName)}
                          </span>
                          <div>
                            <strong>{studentName}</strong>
                            <span>
                              {enrollment.studentEmail || "Student account"}
                            </span>
                            {enrollment.studentPhone ? (
                              <small>{enrollment.studentPhone}</small>
                            ) : null}
                          </div>
                        </div>
                        <div className="enrollment-admin-card__heading">
                          <span
                            className={`enrollment-admin-status is-${toKebabCase(enrollment.status)}`}
                          >
                        ...
                        <div className="enrollment-admin-card__actions">
                          {enrollment.status !== "Active" ? (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                void updateEnrollment(enrollment, 2)
                              }
                            >
                              Activate
                            </button>
                          ...
                          {enrollment.status !== "Cancelled" ? (
                            <button
                              className="enrollment-admin-button"
                              type="button"
                              onClick={() =>
                                void updateEnrollment(enrollment, 4)
                              }
                            >
                              Cancel
                            </button>
                      ...
                      <div className="enrollment-admin-card__grid">
                        <div className="enrollment-admin-detail">
                          <span>Enrollment date</span>
                          <strong>{formatDate(enrollment.enrolledAt)}</strong>
                          <small>Cycle {enrollment.accessCycle || 1}</small>
                        </div>
                        <div className="enrollment-admin-detail">
                          ...
                          <small>
                            {enrollment.programPlanCode || "Plan pricing"}
                          </small>
                        </div>
                        <div className="enrollment-admin-detail">
                        ...
                        <div
                          className={`enrollment-admin-detail ${enrollment.balanceAmount > 0 ? "is-warning" : "is-success"}`}
                        >
                          <span>Balance due</span>
                          <strong>
                            {formatCurrency(enrollment.balanceAmount)}
                          </strong>
                          <small>
                      ...
                      <div className="enrollment-admin-card__footer">
                        <div className="enrollment-admin-progress">
                          <div className="enrollment-admin-progress__label">
                            <span>Payment progress</span>
                            <strong>{paidPercent}%</strong>
                          </div>
                          <div className="enrollment-admin-progress__track">
                            <span style={{ width: `${paidPercent}%` }} />
                          </div>
                        </div>
                        <div
                          className={`enrollment-admin-access ${enrollment.isAccessExpired ? "is-expired" : ""}`}
                        >
                          <span>
                            <GraduationCap size={16} /> {accessLabel}
                          </span>
                          {enrollment.lockedReason ? (
                            <small>{enrollment.lockedReason}</small>
                          ) : (
                            <small>
                              {enrollment.hasFullAccess
                                ? "Projects, reviews and certificate enabled"
                                : "Full payment unlocks all course content"}
                            </small>
                          )}
                        </div>
                      </div>
                      ...
                      <details className="enrollment-admin-payments">
                        <summary>
                          ...
                          <strong>
                            {relatedPayments.length} transaction
                            {relatedPayments.length === 1 ? "" : "s"}
                          </strong>
                        </summary>
                        <div className="enrollment-admin-payment-list">
                          {relatedPayments.length === 0 ? (
                            <p className="enrollment-admin-payment-empty">
                              No payment transactions are linked to this
                              enrollment yet.
                            </p>
                          ) : (
                            relatedPayments.map((payment) => (
                              <div
                                className="enrollment-admin-payment"
                                key={payment.id}
                              >
                                <div>
                                  <strong>
                                ...
                                <div>
                                  <span
                                    className={`enrollment-admin-payment-status is-${toKebabCase(payment.status)}`}
                                  >
          ...
          <section className="lms-list-panel lms-list-panel--wide payments-admin-panel">
            <div className="payments-admin-heading">
              ...
              <div className="payments-admin-controls">
                ...
                <label className="payments-admin-search">
                  <Search size={16} />
                  <span className="sr-only">Search payments</span>
                  <input
                    value={paymentSearch}
                    onChange={(event) => setPaymentSearch(event.target.value)}
                    placeholder="Search student, program, invoice..."
                  />
                </label>
                <label className="payments-admin-filter">
                  <SlidersHorizontal size={16} />
                  <span className="sr-only">Filter payments by status</span>
                  <select
            ...
            <div className="payments-admin-kpis" aria-label="Payment summary">
              <article>
                <span>
                  <CircleDollarSign size={16} /> Collected
                </span>
                <strong>
                  {formatCurrency(
                    paymentStats.verified.reduce(
                ...
                <small>
                  {paymentStats.verified.length} verified payment
                  {paymentStats.verified.length === 1 ? "" : "s"}
                </small>
              </article>
              <article className="is-pending">
                <span>
                  <RefreshCw size={16} /> Pending review
                </span>
                <strong>
                  {formatCurrency(
                    paymentStats.pending.reduce(
                      (total, payment) => total + payment.amount,
                      0,
                    ),
                  )}
                ...
                <small>
                  {paymentStats.pending.length} payment
                  {paymentStats.pending.length === 1 ? "" : "s"} need action
                </small>
              </article>
              <article className="is-discount">
                <span>
                  <BadgePercent size={16} /> Discounts
                </span>
                <strong>
                  {formatCurrency(
                    paymentStats.verified.reduce(
                      (total, payment) => total + payment.discountAmount,
                      0,
                    ),
                  )}
                </strong>
                <small>Applied on verified payments</small>
              </article>
              <article className="is-failed">
                <span>
                  <X size={16} /> Failed attempts
                </span>
                <strong>{paymentStats.failed.length}</strong>
                <small>Kept for payment history</small>
              </article>
            </div>
            ...
            <div className="payments-admin-list">
              {visiblePayments.length === 0 ? (
                <div className="admin-reference-empty">
                  ...
                  <button
                    className="primary-action"
                    onClick={() => setWorkflowInfo("payment")}
                  >
                    <CreditCard size={17} />
              ...
              {visiblePayments.map((payment) => {
                ...
                return (
                  <article
                    key={payment.id}
                    className={`payment-admin-card is-${toKebabCase(payment.status)}`}
                  >
                    <div className="payment-admin-card__header">
                      <div className="payment-admin-student">
                        ...
                        <div>
                          <strong>{studentName}</strong>
                          <span>
                            {payment.studentEmail || "Student account"}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="payment-admin-card__actions">
                        ...
                        {isPending ? (
                          <button
                            className="enrollment-admin-button"
                            type="button"
                            onClick={() => void failPayment(payment)}
                          >
                            Mark failed
                          </button>
                        ) : null}
                        {payment.status === "Verified" ? (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              void openPaymentReceipt(payment.id, "admin")
                            }
                          >
                    ...
                    <div className="payment-admin-card__grid">
                      <div>
                        ...
                        <small>
                          {payment.status === "Verified"
                            ? `Verified ${formatDateTime(payment.verifiedAt ?? payment.createdAt)}`
                      ...
                      <div>
                        ...
                        <strong>
                          {payment.programTitle || "Program unavailable"}
                        </strong>
                        <small>
                          {payment.programPlanName || "Plan unavailable"}
                        </small>
                      </div>
                      <div>
                        <span>Payment method</span>
                        <strong>{payment.gateway || "Not selected"}</strong>
                        <small>
                          Order {payment.gatewayOrderId || "Not generated"}
                        </small>
                      </div>
                      <div>
                        <span>Coupon & pricing</span>
                        <strong>{payment.couponCode || "No coupon"}</strong>
                        <small>
                          {payment.discountAmount > 0
                            ? `${formatCurrency(payment.discountAmount)} off ${formatCurrency(payment.originalAmount)}`
                    ...
                    <div className="payment-admin-card__footer">
                      <span>
                        {payment.invoiceNumber
                          ? `Invoice ${payment.invoiceNumber}`
                          : "Invoice generated after verification"}
                      </span>
                      <span>
                        {payment.gatewayPaymentId
                          ? `Payment ID ${payment.gatewayPaymentId}`
                          : "Gateway payment ID pending"}
                      </span>
          ...
          <section className="lms-list-panel lms-list-panel--wide">
            ...
            >
              <div className="coupon-admin-form__grid">
                ...
                <label>
                  ...
                  <input
                    name="description"
                    placeholder="Early learner launch discount"
                    required
                  />
                </label>
                <label>
                ...
                <label>
                  Eligible audience
                  <select name="audienceType" defaultValue="1">
                    <option value="1">All students</option>
                    <option value="2">New students</option>
                    <option value="3">Existing students</option>
              ...
              <div className="coupon-admin-form__targets">
                <label>
                  Target categories
                  <select name="targetCategoryIds" multiple size={4}>
                    {categories.map((category) => (
                      <option key={category.id} value={category.id}>
                        {category.name}
                      </option>
                  ...
                  <small>Leave empty for every category.</small>
                </label>
                <label>
                  Target programs
                  <select name="targetProgramIds" multiple size={4}>
                    {programs.map((program) => (
                      <option key={program.id} value={program.id}>
                        {program.title}
                      </option>
                  ...
                  <small>Leave empty for every program.</small>
                </label>
                <label>
                  Selected student accounts
                  <select name="targetStudentIds" multiple size={4}>
                  ...
                  <small>Use this with “Selected students”.</small>
                </label>
                <label>
                  Student emails
                  <textarea
                  ...
                  />
                  <small>
                    Comma, semicolon, or newline separated. Works even when the
                    student is not on the current page.
                  </small>
                </label>
              </div>
              <p className="coupon-admin-form__note">
                Coupons are validated server-side and apply only to
                remaining-balance payments. The initial reserve payment,
                including ₹1,500, is never discounted.
              </p>
              <button className="primary-action" type="submit">
                Create coupon
              </button>
            ...
            <div className="lms-scroll-list">
              {adminCoupons.length === 0 ? (
                <div className="table-state">No coupons yet.</div>
              ) : null}
        ...
        {showModule("Certificates") ? (
          <section className="lms-list-panel lms-list-panel--wide">
            <div className="certificate-admin-heading">
              <div>
                <span className="eyebrow">CERTIFICATE ISSUANCE</span>
                <h3>Create certificate</h3>
                <p>
                  Select a fully paid enrollment, complete the certificate
                  details, preview it, then issue.
                </p>
              </div>
              <Award size={28} />
            </div>
            ...
            >
              <div className="certificate-admin-fields">
                <label>
                  ...
                  <select
                    required
                    value={certificateDraft.enrollmentId}
                    onChange={(event) =>
                      selectCertificateEnrollment(event.currentTarget.value)
                    }
                  >
                    <option value="">Select a fully paid enrollment</option>
                    {adminEnrollments
                      .filter(
                        (enrollment) =>
                          enrollment.hasFullAccess &&
                ...
                <label>
                  ...
                  >
                    <option value={1}>Training Certificate</option>
                    <option value={2}>Internship Certificate</option>
                  </select>
                </label>
                <label>
                  Student full name
                  <input
                    required
                    value={certificateDraft.studentName}
                    onChange={(event) =>
                      setCertificateDraft((current) => ({
                        ...current,
                ...
                <label>
                  ...
                  <input
                    readOnly
                    value={
                      adminEnrollments.find(
                        (item) => item.id === certificateDraft.enrollmentId,
                      )?.programTitle ?? "Select an enrollment"
                    }
                  />
                </label>
                <label>
                  ...
                  <input
                    required
                    type="date"
                    value={certificateDraft.fromDate}
                    onChange={(event) =>
                      setCertificateDraft((current) => ({
                        ...current,
                ...
                <label>
                  Director name
                  <input
                    value={certificateDraft.authorizedSignatory}
                    onChange={(event) =>
                      setCertificateDraft((current) => ({
                        ...current,
                        authorizedSignatory: event.currentTarget.value,
                      }))
                    }
                ...
                <label>
                  Signature label
                  <input
                    value={certificateDraft.signatureText}
                    onChange={(event) =>
                      setCertificateDraft((current) => ({
                        ...current,
                        signatureText: event.currentTarget.value,
                      }))
                    }
              ...
              <button
                ...
                type="submit"
                disabled={
                  isIssuingCertificate || !certificateDraft.enrollmentId
                }
              >
            ...
            <div className="lms-scroll-list">
              {adminCertificates.length === 0 ? (
                <div className="table-state">No certificates issued yet.</div>
              ) : null}
              {adminCertificates.map((certificate) => (
                <article key={certificate.id} className="lms-list-item">
                  <div>
                    ...
                    <span>
                      {certificate.programTitle} - {certificate.type}
                    </span>
                  </div>
                  <div className="lms-row-actions">
          ...
          <section className="lms-list-panel lms-list-panel--wide">
            ...
            <div className="lms-scroll-list">
              {adminNotifications.length === 0 ? (
                <div className="table-state">No notifications yet.</div>
              ) : null}
                ...
                <article key={notification.id} className="lms-list-item">
                  <div>
                    ...
                    <span>
                      ...
                      {notification.userEmail ?? notification.userId}
                    </span>
                  </div>
...
function AdminPeoplePanel({
  ...
  const pendingCount = users.filter(
    (user) => user.accountStatus === "PendingEmailVerification",
  ).length;
  ...
  const startResult =
    totalCount === 0
      ? 0
      : ((page?.page ?? query.page) - 1) * query.pageSize + 1;
  const endResult =
    totalCount === 0 ? 0 : Math.min(startResult + users.length - 1, totalCount);
  ...
  async function copyTemporaryPassword() {
    try {
      ...
      onMessage({
        tone: "error",
        text: "Could not copy password. Copy it manually from the field.",
      });
    }
  }
  ...
  return (
    <section className={`admin-people-page admin-people-page--${roleLabel}`}>
      <div className="admin-people-hero">
        <div className="admin-people-hero__copy">
          ...
          <div>
            ...
            <h2>{title}</h2>
            <p>
              Add learners, review onboarding, and keep account access under
              control.
            </p>
          </div>
        </div>
        ...
        <div
          className="admin-people-hero__metrics"
      ...
      {isAdding ? (
        <form
          className="dashboard-card admin-people-create"
      ...
      <div className="admin-people-toolbar">
        ...
        <label>
          ...
          <select
            value={query.status}
            onChange={(event) =>
              changeStatus(event.target.value as PeopleStatusFilter)
            }
          >
        ...
        <button
          className="admin-students-reset secondary-action"
          onClick={() => {
            setSearchDraft("");
            onQueryChange({ ...initialPeopleQuery });
          }}
        >
          Reset filters
        </button>
      ...
      <div className="admin-people-list">
        {isLoading ? (
          <div className="table-state">Loading {roleLabel}s...</div>
        ) : null}
        {!isLoading && users.length === 0 ? (
          <div className="admin-reference-empty">
            ...
            <h3>
              {query.search || query.status !== initialPeopleQuery.status
            ...
            <p>
              {query.search
                ? emptyText
                : "Start by adding your first student and give them access to learn, practice and grow."}
            </p>
            <button
              className="primary-action"
        ...
        {!isLoading
          ? users.map((user) => {
              ...
              const profilePhotoUrl = user.profilePhotoUrl
                ? toApiFileUrl(user.profilePhotoUrl)
                : "";
              ...
              return (
                ...
                >
                  ...
                  >
                    <span
                      className={`admin-person-card__avatar ${profilePhotoUrl ? "has-photo" : ""}`}
                    >
                      {profilePhotoUrl ? (
                        <img
                          src={profilePhotoUrl}
                  ...
                  <div className="admin-person-card__actions">
                    ...
                    <button
                      ...
                      disabled={
                        isCurrentUser ||
                        actionUserId ===
                          `${user.id}-${isLocked ? "unlock" : "lock"}`
                      }
                      onClick={() =>
                        void onUserAction(user, isLocked ? "unlock" : "lock")
                      }
                  ...
                  {isEditing ? (
                    <form
                      ...
                      onSubmit={(event) => void submitEdit(event, user)}
                    >
                      <label>
                        Full name
                        <input
                      ...
                      <div className="admin-person-edit__actions">
                        ...
                        <button
                          className="primary-action"
                          type="submit"
                          disabled={actionUserId === `${user.id}-edit`}
                        >
                          <Save size={17} />
                          {actionUserId === `${user.id}-edit`
                            ? "Saving"
                            : "Save"}
                        </button>
                      </div>
...
function StudentDashboard({
  ...
  const [dashboard, setDashboard] =
    useState<StudentLmsDashboardResponse | null>(null);
  const [workspace, setWorkspace] =
    useState<StudentProgramWorkspaceResponse | null>(null);
  const [myPrograms, setMyPrograms] =
    useState<StudentMyProgramsResponse | null>(null);
  ...
  >({});
  const [projectSubmissionFiles, setProjectSubmissionFiles] = useState<
  ...
  function previewAction() {
    ...
    setMessage({
      tone: "success",
      text: "Design preview: this action is available after signing in to a student account.",
    });
    return true;
  }
  ...
  async function enroll(program: ProgramSummaryResponse, mode: 1 | 2) {
    ...
    try {
      ...
      const plan =
        details.data.plans.find(
          (item) => item.code === "INTERMEDIATE" && item.isActive,
  ...
  async function payBalance(
    mode: 1 | 2 | 3,
    targetEnrollment?: EnrollmentResponse,
  ) {
    if (previewAction()) return;
    const enrollment =
      targetEnrollment ?? workspace?.enrollment ?? dashboard?.enrollment;
    if (!enrollment) {
      return;
    }
  ...
  async function submitProject(projectId: string) {
    ...
    if (!value && !notes && !file) {
      setMessage({
        tone: "error",
        text: "Attach your project work, add a project link, or write submission notes.",
      });
      return;
    }
    ...
    try {
      ...
      await studentLmsApi.submitProject(projectId, {
        gitHubUrl: value || undefined,
        fileAssetId,
      ...
      <>
        ...
        <StudentOverview
          name={
            preview
              ? "Student"
              : (auth.user?.fullName?.split(" ")[0] ?? "Student")
          }
          dashboard={dashboard}
          workspace={workspace}
          loading={isLoading}
          openModule={openModule}
          continueLearning={(programId) => {
            if (!previewAction()) navigate(`/learning/${programId}`);
          }}
        />
      </>
    );

  if (activeModule === "Certificates" || activeModule === "Payments")
    return (
      <section className="student-module-page">
        <StudentModuleHeader
          module={activeModule}
          onDashboard={() => openModule("Overview")}
        />
        <ToastMessage message={message} onDismiss={() => setMessage(null)} />
        {isLoading ? (
          <div className="student-module-surface" role="status">
            Loading your {activeModule.toLowerCase()}...
          </div>
        ) : activeModule === "Certificates" ? (
          <StudentCertificates
            certificates={certificates}
            onProgram={() => openModule("My Program")}
          />
        ) : (
          <StudentPayments
            enrollment={enrollment}
            payments={payments}
            onReceipt={(id) => void viewReceipt(id)}
            onPay={() =>
              void payBalance(
                enrollment?.isAccessExpired || !enrollment?.paidAmount ? 1 : 3,
              )
            }
            actionId={actionId}
          />
    ...
    (project) =>
      ...
      (projectFilter === "In Progress"
        ? !project.latestSubmission ||
          ["Draft", "NeedsRevision"].includes(project.latestSubmission.status)
        : projectFilter === "Submitted"
          ? project.latestSubmission?.status === "Submitted"
          : ["Approved", "Rejected", "NeedsRevision"].includes(
  ...
  return (
    <section
      className={`dashboard-stack ${activeModule === "Projects" ? "student-module-page student-projects-page" : ""}`}
    >
      {activeModule === "Projects" && (
        <>
          <StudentModuleHeader
            ...
            onDashboard={() => openModule("Overview")}
          />
          <div className="student-project-tabs" aria-label="Filter projects">
            {["All Projects", "In Progress", "Submitted", "Reviewed"].map(
              (filter) => (
                <button
                  key={filter}
      ...
      {showOverview ? (
        <section className="metric-grid">
          <DashboardMetric
            ...
            value={dashboard?.programStatus ?? "-"}
          />
          <DashboardMetric
            icon={BarChart3}
            label="Learning progress"
            value={`${dashboard?.learningProgressPercentage ?? 0}%`}
          />
          <DashboardMetric
            icon={FolderKanban}
            label="Pending projects"
            value={dashboard?.pendingProjects ?? 0}
          />
          <DashboardMetric
            icon={WalletCards}
            label="Balance due"
            value={formatCurrency(dashboard?.balanceDue ?? 0)}
          />
        </section>
      ) : null}
      ...
      {isLoading ? (
        <section className="dashboard-card">
          <div className="table-state">Loading LMS workspace...</div>
        </section>
      ) : null}
      ...
      {!isLoading && activeModule === "Profile" ? (
        <section className="dashboard-card student-program-hero">
          <div>
            ...
            <p>
              Add personal, academic, resume, LinkedIn, GitHub, portfolio,
              skills, and target role details whenever they are ready.
            </p>
          </div>
          <button
            className="primary-action"
        ...
        <section className="dashboard-card student-marketplace">
          <div className="card-title-row">
            <div>
              ...
              <p>
                Each program includes curriculum, projects, and certification.
              </p>
            </div>
            <Zap size={23} />
          </div>
      ...
      {!isLoading && activeModule === "My Program" ? (
        <StudentMyProgramLibrary
          ...
          onPay={(courseEnrollment, mode) =>
            void payBalance(mode, courseEnrollment)
          }
        />
      ) : null}
          ...
          {showStudentModule("My Program") ? (
            <section className="dashboard-card student-program-hero">
              <div>
                ...
                <p>
                  {enrollment.lockedReason ??
                    "Full LMS access is active. Keep building proof through lessons and reviews."}
                </p>
              </div>
              <div className="student-progress-block">
                <strong>{dashboard?.learningProgressPercentage ?? 0}%</strong>
                <span>
                  {dashboard?.completedLessons ?? 0} of{" "}
                  {dashboard?.totalLessons ?? 0} lessons complete
                </span>
              </div>
            </section>
          ) : null}
              ...
              <div className="card-title-row">
                <div>
                  ...
                  <p>
                    ...
                    {formatCurrency(enrollment.paidAmount)} of{" "}
                    {formatCurrency(enrollment.totalAmount)}.
                  </p>
                  <small>
                    {enrollment.isAccessExpired
                      ? "Access expired — renew to continue."
                      : enrollment.accessExpiresAt
                        ? `Access until ${formatDateTime(enrollment.accessExpiresAt)}.`
                        : "Access begins after payment verification."}
                  </small>
                </div>
                <CreditCard size={23} />
              </div>
                ...
                <div className="payment-actions">
                  {enrollment.isAccessExpired || enrollment.paidAmount <= 0 ? (
                    <button
                      className="primary-action"
                      type="button"
                      onClick={() => void payBalance(1)}
                      disabled={actionId === "payment-1"}
                    >
                    ...
                    <button
                      className="primary-action"
                      type="button"
                      onClick={() => void payBalance(3)}
                      disabled={actionId === "payment-3"}
                    >
              ...
              <div className="submission-list">
                {!enrollment.hasFullAccess ? (
                  <div className="access-locked-notice">
                    <Lock size={18} />
                    <div>
                      <strong>Projects are locked</strong>
                      <span>
                        {enrollment.isAccessExpired
                          ? "Renew your six-month access first."
                          : "Pay the remaining balance to submit projects and unlock your certificate."}
                      </span>
                    </div>
                    <button
                      className="secondary-action"
                ...
                {enrollment.hasFullAccess && projects.length === 0 ? (
                  <div className="project-empty-state">
                    <FolderKanban size={24} />
                    <strong>No projects assigned yet.</strong>
                    <p>
                      Your assigned project work will appear here when the admin
                      publishes it to you.
                    </p>
                  </div>
                ) : null}
                {filteredProjects.length === 0 && (
                  <div className="student-module-empty">
                    ...
                    <p>Choose another filter to see your assigned work.</p>
                  </div>
                )}
                  ...
                  return (
                    <article key={project.id} className="student-project-card">
                      <div className="student-project-card__header">
                        <div>
                          <span className="eyebrow">Assigned project</span>
                          <h3>{project.title}</h3>
                        </div>
                        {submission ? (
                          <span
                            className={`project-review-status is-${submission.status.toLowerCase()}`}
                          >
                          ...
                          <span className="project-review-status is-pending">
                            Not submitted
                          </span>
                        )}
                      </div>
                      <p className="student-project-card__description">
                        {project.description}
                      </p>
                      <div className="student-project-meta">
                        ...
                        <small>
                          <Trophy size={14} /> {project.maxScore} grade points
                        </small>
                      </div>
                      {project.requiredArtifacts.length > 0 ? (
                        <div className="student-project-requirements">
                          <strong>Submit</strong>
                          {project.requiredArtifacts.map((artifact) => (
                            <span key={artifact}>{artifact}</span>
                          ))}
                        </div>
                      ) : null}
                      {(project.usefulLinks?.length ?? 0) > 0 ||
                      project.referenceMediaUrl ? (
                        <div className="student-project-resources">
                          ...
                          {project.referenceMediaUrl ? (
                            <a
                              ...
                              rel="noreferrer"
                            >
                              <FileText size={13} /> Reference media
                            </a>
                          ) : null}
                        </div>
                      ) : null}
                        ...
                        <div className="student-project-review">
                          <div>
                            <strong>
                              ...
                              submission.score !== null
                                ? `Awarded ${submission.score} / ${project.maxScore}`
                                : "Admin feedback"}
                            </strong>
                          ...
                          {submission.fileAssetId ? (
                            <button
                              type="button"
                              onClick={() =>
                                void openStudentSubmissionFile(
                                  submission.fileAssetId!,
                                )
                              }
                            >
                      ...
                      <div className="student-project-submit-form">
                        <label>
                          <span>
                            GitHub or project link <small>optional</small>
                          </span>
                          <input
                            ...
                            disabled={!enrollment.hasFullAccess}
                          />
                        </label>
                        <label className="student-project-file">
                          <span>
                            Project work / media{" "}
                            <small>PDF, ZIP, image, video · max 25 MB</small>
                          </span>
                          <span className="student-project-file__control">
                            <UploadCloud size={17} />
                            <span>
                              {selectedFile?.name ?? "Choose your project file"}
                            </span>
                            <input
                              type="file"
                              accept="image/*,video/*,.pdf,.doc,.docx,.ppt,.pptx,.zip,.txt,.md"
                              disabled={!enrollment.hasFullAccess}
                              onChange={(event) => {
                                const file = event.target.files?.[0] ?? null;
                                if (file && file.size > 25 * 1024 * 1024) {
                                  setMessage({
                                    tone: "error",
                                    text: "Project files must be 25 MB or smaller.",
                                  });
                                  event.currentTarget.value = "";
                                  return;
                                }
                                setProjectSubmissionFiles((items) => ({
                                  ...items,
                                  [project.id]: file,
                                }));
                              }}
                        ...
                        <label>
                          <span>
                            Notes for admin{" "}
                            <small>explain your work or add context</small>
                          </span>
                          <textarea
                            value={projectSubmissionNotes[project.id] ?? ""}
                            onChange={(event) =>
                              setProjectSubmissionNotes((items) => ({
                                ...items,
                                [project.id]: event.target.value,
                              }))
                            }
                            placeholder="Tell the reviewer what you built, what to test, and anything you want feedback on..."
                            disabled={!enrollment.hasFullAccess}
                          />
                        </label>
                        <button
                          className="primary-action"
                          type="button"
                          onClick={() => void submitProject(project.id)}
                          disabled={
                            !enrollment.hasFullAccess ||
          ...
          {showStudentModule("Certificates") ? (
            <section className="dashboard-card lms-side-panel">
              ...
              {certificates.length === 0 ? (
                <p>Certificates will appear after project completion.</p>
              ) : null}
              {certificates.map((certificate) => (
                <article key={certificate.id} className="certificate-mini">
                  ...
                  <div>
                    <strong>{certificate.certificateId}</strong>
                    <span>
                      {certificate.status} - {certificate.type}
                    </span>
                  </div>
                </article>
              ))}
...
function toApiAcceptableThumbnailUrl(value: string) {
  ...
  try {
    const oldLocalAssetMatch = trimmed.match(
      /\/api\/v1\/assets\/local-files\/([0-9a-f-]{36})/i,
    );
    if (oldLocalAssetMatch?.[1]) {
      return normalizeThumbnailUrl(
        new URL(
          `/api/v1/assets/public-files/${oldLocalAssetMatch[1]}`,
          env.apiBaseUrl,
        ),
      );
    }
...
function toIndiaMobileDigits(phone?: string) {
  ...
  const compact = phone.replace(/\D/g, "");
  return compact.startsWith("91") && compact.length === 12
    ? compact.slice(2)
    : compact;
}
